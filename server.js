...
let database = {
  users: {}, articles: {}, comments: {}, nextArticleId: 1, nextCommentId: 1
};
...
function postComment(url, request) {
  ...
  if (requestComment && requestComment.body && requestComment.username && requestComment.articleId && database.users[requestComment.username] && database.articles[requestComment.articleId]) {//evaluates the desired edge cases
    const comment = {
  ...
  return response;//gotta return the response!
}
...
function updateComment(url, request) {
  ...
  if(!id || !requestComment || !requestComment.body){
    ...
    response.status = 404;//if there is no comment, return 404 "NOT FOUND"
  } else {
...
function deleteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);//retrieve id
  const savedComment = database.comments[id];//retrieve current stored comment
  const response = {};
  ...
  if (savedComment) {
    ...
    response.status = 204;//204 means "SUCCESS" but returns no content
  } else {
...
function upvoteComment(url, request) {
  ...
  if(id && database.users[requestUsername] && savedComment){
    if (savedComment.upvotedBy.includes(requestUsername)) {
      response.status = 400;
      return response;
    } else if (savedComment.downvotedBy.includes(requestUsername)) {
      savedComment.downvotedBy.splice(savedComment.downvotedBy.indexOf(requestUsername, 1));
      savedComment.upvotedBy.push(requestUsername);
      response.status = 200;
      response.body = {comment: savedComment};
    }
    savedComment.upvotedBy.push(requestUsername);
...
function downvoteComment(url, request) {
  ...
  if(id && database.users[requestUsername] && savedComment){
    if (savedComment.downvotedBy.includes(requestUsername)) {
      response.status = 400;
      return response;
    } else if (savedComment.upvotedBy.includes(requestUsername)) {
      savedComment.upvotedBy.splice(savedComment.downvotedBy.indexOf(requestUsername, 1));
      savedComment.downvotedBy.push(requestUsername);
